...
bool idaapi outop(op_t &x)
{
  switch ( x.type )
  {
    ...
    case o_imm:
    {
      uchar sign = cmd.itype == TMS6_mvkh
                || cmd.itype == TMS6_mvklh
                || (cmd.itype == TMS6_mvk && is_mvk_scst16_form(cmd.ea))
                 ? 0
                 : OOF_SIGNED;
      outval(x, OOFS_IFSIGN|OOFW_IMM|sign);
      break;
    }
...
//
  ...
  if ( cmd.Op1.shown() ) out_one_operand(0);

  if ( cmd.Op2.type != o_void && cmd.Op2.shown() )
  {
    out_symbol(',');
    OutChar(' ');
    out_one_operand(1);
  ...
  if ( cmd.Op3.type != o_void && cmd.Op3.shown() )
  {
    out_symbol(',');
    OutChar(' ');
    out_one_operand(2);
  ...
  if ( (cmd.cflags & aux_para) == 0 )
  {
    ...
    switch ( target )
    {
      ...
      default:
        if ( target != 0 )
        {
          qstring nbuf = get_colored_name(target & ~1);
...
void idaapi segstart(ea_t ea)
{
  ...
  if ( strcmp(sname,".bss") == 0 )
    return;
  if ( strcmp(sname,".text") == 0 || strcmp(sname,".data") == 0 )
...
//--------------------------------------------------------------------------
bool idaapi outspec(ea_t ea, uchar stype)
{
  qstring nbuf;
