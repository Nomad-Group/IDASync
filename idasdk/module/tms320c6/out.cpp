...
{
  out_tms320c6_t(void) : outctx_t(BADADDR) {} // not used
public:
...
//
  ...
  {
    nodeidx_t ndx = tnode.altval_ea(ea);
    switch ( ndx )
    {
      case 1:
        gen_printf(inf.indent, COLSTR("; BRANCH OCCURS", SCOLOR_AUTOCMT));
        break;
      case 2:
        gen_printf(inf.indent, COLSTR("; CALL OCCURS", SCOLOR_AUTOCMT));
        break;
      default:
        if ( ndx != 0 )
        {
          ea_t target = node2ea(ndx);
...
bool idaapi outspec(outctx_t &ctx, uchar stype)
{
  ea_t ea = ctx.insn_ea;
  qstring nbuf;
