...
{
  out_i51_t(void) : outctx_t(BADADDR) {} // not used
public:
...
bool out_i51_t::out_operand(const op_t &x)
{
  ...
  switch ( x.type )
  {
    ...
    case o_near:
      v = x.type == o_near
        ? map_code_ea(insn, x)
        : i51_map_data_ea(insn, x.addr, x.n);
...
/*        int nbit;
    ...
    case o_bit:
      ...
      if ( (dir & 0x80) == 0 )
        dir = dir/8 + 0x20;
OUTBIT:
      v = i51_map_data_ea(insn, dir, x.n);
      out_addr_tag(v);
...
{
  ctx.gen_header(GH_PRINT_ALL, device.c_str(), deviceparams.c_str());
}
...
void idaapi i51_segstart(outctx_t &ctx, segment_t *Sarea)
{
  char buf[MAXSTR];
...
static void do_out_equ(outctx_t &ctx, const char *name, const char *equ, uchar off)
{
