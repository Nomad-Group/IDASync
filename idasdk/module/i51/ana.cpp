...
static int ana_basic(void)
{
  ...
  if ( mx_a5 )
  {
    ...
    {
      cmd.itype = I51_add;
      op_rd(cmd.Op1, (nibble0 & 4) == 0 ? rPR0 : rPR1, dt_dword);
      int val = nibble0 & 3;
      op_im(cmd.Op2, val == 0 ? 4 : val, dt_byte);
      return cmd.size;
    }
    if (nibble1 == 0x4 && nibble0 >= 0x8 ) // EMOVE A, @PRi+#data2
    {
      ...
      opAcc();
      int val = nibble0 & 3;
      op_ph(cmd.Op2, (nibble0 & 4) == 0 ? fPr0 : fPr1, dt_dword, val == 0 ? 4 : val);
      return cmd.size;
    }
    if (nibble1 == 0x5 && nibble0 >= 0x8 ) // EMOVE @PRi+#data2, A
    {
      cmd.itype = I51_emov;
      int val = nibble0 & 3;
      op_ph(cmd.Op1, (nibble0 & 4) == 0 ? fPr0 : fPr1, dt_dword, val == 0 ? 4 : val);
      opAcc(cmd.Op2);
      return cmd.size;
    }
  }
    ...
    switch ( nibble0 )
    {
      case 0:
        ...
        switch ( nibble1 )
        {
          ...
          case 0x9: // mov
            cmd.Op1.type = o_reg;
            cmd.Op1.reg = mx_a5 ? rEptr : rDptr;
            cmd.Op1.dtyp = mx_a5 ? dt_dword : dt_word;
            cmd.Op2.type  = o_imm;
            cmd.Op2.offb  = (uchar)cmd.size;
            if ( mx_a5 )
            {
              cmd.Op2.value = (((ea_t)ua_next_word()) << 8) + ua_next_byte();;
          ...
          case 0xE: // movx
            opAcc(cmd.Op1);
            cmd.Op2.type = o_phrase;
            cmd.Op2.phrase = mx_a5 ? fEptr : fDptr;;
            break;
          case 0xF: // movx
            opAcc(cmd.Op2);
            cmd.Op1.type = o_phrase;
            cmd.Op1.phrase = mx_a5 ? fEptr : fDptr;
            break;
        }
        break;
      case 1: // acall, ajmp
      ...
      case 2:
        ...
        switch ( nibble1 ) {
          case 0x0: case 0x1: // ljump (ejmp), lcall (ecall)
            ...
            if ( mx_a5 ) // ecall
            {
              cmd.itype = nibble1 == 0 ? I51_ejmp : I51_ecall;
              cmd.Op1.addr = (((ea_t)ua_next_word()) << 8) + ua_next_byte();
              if ( cmd.Op1.addr >= 0x800000 )
                cmd.Op1.addr -= 0x800000;
            }
            else
            {
      ...
      case 3:
        ...
        switch ( nibble1 ) {
          ...
          case 0x7: // jmp
            cmd.Op1.type = o_phrase;
            cmd.Op1.phrase = mx_a5 ? fAeptr : fAdptr;
            break;
          case 0x8: case 0x9: // movc
            opAcc();
            cmd.Op2.type = o_phrase;
            cmd.Op2.phrase =  nibble1 == 0x8 ? fApc : (mx_a5 ? fAeptr : fAdptr);
            break;
          case 0xA: // inc
            cmd.Op1.type = o_reg;
            cmd.Op1.reg = mx_a5 ? rEptr : rDptr;
