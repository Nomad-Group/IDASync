...
static void set_phrase(op_t &op, st9_phrases phrase, uint16 reg1, int reg2 = -1)
{
  op.type = o_phrase;
  op.reg = reg1;
  op.specflag2 = phrase;
...
{
  QASSERT(10035, type == o_mem || type == o_near); // bad optype_t in set_addr()
  op.type = type;
  op.addr = addr;
  op.dtype = dt_word;
}
...
static void fill_cmd(insn_t &insn, int byte, const opcode *op)
{
  ...
  switch ( op->addr )
  {
    ...
    // RRd,RRs
    case A_grpd_grps:
      set_reg(insn.Op1, rRR0 + (insn.get_next_byte() & 0xFE));
      set_reg(insn.Op2, rRR0 + (insn.get_next_byte() & 0xFE));
      break;
    ...
    case A_grdi:
    case A_grsi:
      set_reg(insn.Op1, rR0 + insn.get_next_byte(), OP_IS_IND);
      break;
    ...
    case A_grpd_1:
      {
        ...
        if ( (insn.itype == st9_unlink || insn.itype == st9_unlinku)
          && (byte & 0xF0) == 0xD0 )
        {
          reg = rrr0 + (byte & 0x0E);
        }
    ...
    case A_grpi:
    case A_grpdi:
      set_reg(insn.Op1, rRR0 + (insn.get_next_byte() & 0xFE), OP_IS_IND);
      break;
    ...
    case A_8qxgrpi:
      byte = insn.get_next_byte();
      set_displ(insn.Op1, o_mem, char(insn.get_next_byte()), o_reg, rRR0 + (byte & 0xFE));
      break;
    ...
    case A_16qxgrpi:
      ...
      set_displ(insn.Op1, o_mem, short(my_next_word(insn)), o_reg, rRR0 + (byte & 0xFE));
      break;
...
#define BIT_OP_2    (insn.itype != st9_bld ? insn.Op2 : insn.Op1)
    ...
    case A_seg_16q_01:
    case A_seg_16q_11:
      set_imm(insn.Op1, insn.get_next_byte() & 0x3F, dt_byte, OP_IMM_NO_SHIFT);
      set_addr(insn.Op2, o_near, insn.get_next_word());
      break;
