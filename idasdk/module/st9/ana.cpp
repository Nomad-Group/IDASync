...
static uint16 get_alu_insn(const int code)
{
    int insn_id = (code & 0xF0) >> 4;

    QASSERT(10031, cmd.itype == st9_ald || cmd.itype == st9_aldw);
...
static void fill_cmd(int byte, const struct opcode *op) {
    ...
    switch ( op->addr ) {
        ...
        case A_16qxgrpi:
            ...
            set_displ(cmd.Op1, o_mem, short(my_next_word()), o_reg, rRR0 + (byte & 0xFE));
            break;
