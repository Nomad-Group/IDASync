...
// Emulate an operand.
static void handle_operand(const op_t &op, bool lwrite)
{
    switch ( op.type ) {
        // Code address
        case o_near:
...
int idaapi emu(void) {
    ...
    switch ( cmd.itype ) {
        case st9_srp:
            {
                ...
                split_srarea(cmd.ea, rRW, val | (val << 8), SR_auto);
            }
            rw_has_changed = true;
            break;
        ...
        case st9_srp0:
            {
                sel_t RW = get_segreg(cmd.ea, rRW);
                split_srarea(cmd.ea, rRW, cmd.Op1.value | (RW & 0xFF00), SR_auto);
            }
            rw_has_changed = true;
            break;
        ...
        case st9_srp1:
            {
                sel_t RW = get_segreg(cmd.ea, rRW);
                split_srarea(cmd.ea, rRW, (cmd.Op1.value << 8) | (RW & 0x00FF), SR_auto);
            }
            rw_has_changed = true;
            break;

        case st9_spp:
            split_srarea(cmd.ea, rRP, cmd.Op1.value, SR_auto);
            rp_has_changed = true;
            break;
    }
...
bool idaapi create_func_frame(func_t *pfn) {
    ...
    for(int i = 0; true; i++) {
        ...
        int err = add_regvar(pfn, cmd.ea, cmd.ea + cmd.size,
