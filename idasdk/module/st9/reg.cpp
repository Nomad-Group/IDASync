...
static void idaapi gnu_func_header(func_t *pfn)
{
  ...
  {
    printf_line(inf.indent, COLSTR(".desc %s, %s", SCOLOR_ASMDIR), name.begin(),
        pfn->is_far() ? "far" : "near");
    printf_line(inf.indent, COLSTR(".proc %s", SCOLOR_ASMDIR), name.begin());
  }
  gl_xref = 1;
...
//lint -esym(818,pfn)
static void idaapi gnu_func_footer(func_t *)
{
