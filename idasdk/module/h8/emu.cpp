...
static bool get_op_value(const op_t &x, uval_t *value, ea_t *base_addr = NULL)
{
  ...
  if ( ok )
  {
    if ( x.type == o_phrase )
    {
      if      ( x.phtype == ph_pre_inc ) *value += 1;
      else if ( x.phtype == ph_pre_dec ) *value -= 1;
    }
    else if ( x.type == o_displ )
    {
      if ( x.displtype == dt_regidx )
      {
        if      ( (cmd.auxpref == aux_long) != 0 ) *value <<= 2;
        else if ( (cmd.auxpref == aux_word) != 0 ) *value <<= 1;
      }
...
//----------------------------------------------------------------------
static void process_operand(const op_t &x,int isAlt,int isload)
{
  uval_t op_value;
                                ...
                                // the idp module is allowed to modify 'cmd'
{
  ...
  if ( cmd.itype == H8_push && cmd.Op1.reg==ER3 ) return 100;  // push.l er3
  if ( cmd.itype == H8_push && cmd.Op1.reg==R3 ) return 100;  // push.w r3
  return 0;
}
...
{
  static is_pattern_t * const h8_patterns[] =
  {
    is_hew_pattern,
  };
