...
ea_t calc_data_mem(const op_t &op)
{
  ...
  if ( op.tms_regH == DPH )
  {
    dph = get_segreg(toEA(cmd.cs, cmd.ip), DPH);
  ...
  if ( op.tms_regP == DP )
  {
    dp = get_segreg(toEA(cmd.cs, cmd.ip), DP);
...
ea_t calc_io_mem(const op_t &op)
{
  ...
  if ( op.tms_regP == PDP )
  {
    pdp = get_segreg(toEA(cmd.cs, cmd.ip), PDP);
    ...
    case o_reg:
      ...
      if ( !use )
      {
        if ( op.reg == DP || op.reg == DPH || op.reg == PDP )
          split_srarea(get_item_end(cmd.ea), op.reg, BADSEL, SR_auto);
      }
      break;
        ...
        if ( ea != BADADDR )
        {
          ...
          if ( !use )
          {
            ...
            if ( reg == DP || reg == DPH || reg == PDP )
              split_srarea(get_item_end(cmd.ea), reg, BADSEL, SR_auto);
            doVar(ea);
          }
        }
      }
...
int idaapi emu(void)
{
  ...
  if ( (cmd.itype == TMS320C55_bclr1 || cmd.itype == TMS320C55_bset1 )
    ...
    && (cmd.Op1.reg == CPL || cmd.Op1.reg == ARMS))
      split_srarea(get_item_end(cmd.ea), cmd.Op1.reg, cmd.itype == TMS320C55_bclr1 ? 0 : 1, SR_auto);

  // DP, DPH and PDP changes
  if ( cmd.itype == TMS320C55_mov2 && cmd.Op2.type == o_reg && cmd.Op1.type == o_imm )
  {
    if ( cmd.Op2.reg == DP )
      split_srarea(get_item_end(cmd.ea), DP, cmd.Op1.value & 0xFFFF, SR_auto);
    else if ( cmd.Op2.reg == DPH )
      split_srarea(get_item_end(cmd.ea), DPH, cmd.Op1.value & 0x7F, SR_auto);
    else if ( cmd.Op2.reg == PDP )
      split_srarea(get_item_end(cmd.ea), PDP, cmd.Op1.value & 0x1FF, SR_auto);
  }
