...
typedef void (*func_op_mask_t)(const mask_t *, int64 offset, op_t *op, insn_t &insn);
...
static bool process_masks_operand(
        insn_t &insn,
        const mask_t *mask,
        int64 code,
        int64 op_mask_n,
        ...
        bool bTest)
{
  ...
  op_mask->func(mask, opv, &work, insn);
...
#define insn_chars(NAME, CHARS, PARALLEL, NAME1_CHARS, NAME2_CHARS)    \
  static void NAME(const mask_t *, int64 offset, op_t *, insn_t &insn) \
  {                                                                    \
    if ( (offset & 1) != 0 )                                           \
    {                                                                  \
...
// set user parellel
static void insn_UP(const mask_t *, int64, op_t *, insn_t &insn)
{
  insn.SpecialModes |= TMS_MODE_USER_PARALLEL;
}

// built-in parallelism
static void blt_prll(const mask_t *, int64, op_t *, insn_t &insn)
...
#define op_imm(NAME, DTYP)                                                \
  static void op_##NAME(const mask_t *, int64 offset, op_t *op, insn_t &) \
  {                                                                       \
    op->type  = o_imm;                                                    \
...
//--------------------------------------------------------------------------
static void op_min_k4(const mask_t *, int64 offset, op_t *op, insn_t &)
{
  op->type  = o_imm;
  op->value = (uval_t)-offset;
  op->dtype = dt_byte;
...
//--------------------------------------------------------------------------
static void op_K8(const mask_t *, int64 offset, op_t *op, insn_t &)
{
  op->type  = o_imm;
...
//--------------------------------------------------------------------------
static void op_K16(const mask_t *, int64 offset, op_t *op, insn_t &)
{
  op->type  = o_imm;
...
//--------------------------------------------------------------------------
/*
static void op_K23(const mask_t *, int64 offset, op_t *op, insn_t &)
{
  op->type  = o_imm;
...
//--------------------------------------------------------------------------
static void op_1(const mask_t *, int64, op_t *op, insn_t &)
{
  op->type  = o_imm;
  op->value = 1;
  op->dtype = dt_byte;
...
//--------------------------------------------------------------------------
static void op_min_1(const mask_t *, int64, op_t *op, insn_t &)
{
  op->type  = o_imm;
  op->value = uval_t(-1);
  op->dtype = dt_byte;
...
static char optional_op; // index of an optional operand

static void op_src(const mask_t *, int64 offset, op_t *op, insn_t &) // FSSS, FDDD
{
  op->type  = o_reg;
  op->dtype = dt_word;
...
// optional operand
static void opt_src(const mask_t *mask, int64 offset, op_t *op, insn_t &insn) // FSSS, FDDD
{
  if ( optional_op != -1
...
//--------------------------------------------------------------------------
static void op_xsrc(const mask_t *, int64 offset, op_t *op, insn_t &) // XSSS, XDDD
...
//--------------------------------------------------------------------------
static void op_Xmem(const mask_t *mask, int64 offset, op_t *op, insn_t &insn) // XXXMMM, YYYMMM
{
  op_ARx(mask, offset >> 3, op, insn);
  op->tms_modifier = TMS_MODIFIER_REG + (offset & 0x7);
}
#define op_Ymem op_Xmem

//--------------------------------------------------------------------------
static void op_Cmem(const mask_t *mask, int64 offset, op_t *op, insn_t &insn) // mm
{
  op_CDP(mask, 0, op, insn);
    ...
    else
    {
      ...
      if ( (offset & 0x11) == 0x01 ) // xxx0 xxx1
      {
        switch ( bits )
        {
          ...
          case 6: // *ARn(#K16)
          case 7: // *+ARn(#K16)
            op->value = get_signed( (get_byte(insn.ea+insn.size) << 8) | get_byte(insn.ea+insn.size+1), 0xFFFF );
            op->tms_modifier = TMS_MODIFIER_REG_OFFSET + uchar(bits)-6;
            insn.size += 2;
            break;
        }
...
//--------------------------------------------------------------------------
static void op_Smem(const mask_t *mask, int64 offset, op_t *op, insn_t &insn) // AAAAAAAI
{
  op_mem(mask, offset, op, dt_word, insn);
}

//--------------------------------------------------------------------------
static void op_Lmem(const mask_t *mask, int64 offset, op_t *op, insn_t &insn) // AAAAAAAI
{
  op_mem(mask, offset, op, dt_dword, insn);
}

//--------------------------------------------------------------------------
static void op_ARn_mod(const mask_t *mask, int64 offset, op_t *op, insn_t &insn) // AAAAAAAI
{
  op_mem(mask, offset, op, dt_word, insn, true);
}

//--------------------------------------------------------------------------
// @dma = bit number
static void op_Baddr(const mask_t *mask, int64 offset, op_t *op, insn_t &insn) // AAAAAAAI
{
  if ( !(offset & 1) ) // @dma
  {
    op->type  = o_imm;
  ...
  {
    op_mem(mask, offset, op, dt_byte, insn);
  }
}

#define fn_operator(NAME, OPERATOR)                                           \
  static void fn_##NAME(const mask_t *, int64 offset, op_t *op, insn_t &insn) \
  {                                                                           \
    if ( offset & 1 )                                                         \
    {                                                                         \
      get_last_op(op, insn);                                                  \
...
//--------------------------------------------------------------------------
static void op_BitIn(const mask_t *, int64 offset, op_t *op, insn_t &)
{
  op->type  = o_reg;
  op->dtype = dt_byte;
...
//--------------------------------------------------------------------------
static void op_swap(const mask_t *mask, int64 offset, op_t *op, insn_t &insn)
{
  insn.Op1.type  = o_reg;
  ...
  switch ( offset )
  {
    ...
    case 0x10: insn_1_P(mask, 1, op, insn); insn.Op1.reg = AC0; insn.Op2.reg = AC2; break;
    case 0x14: insn_1_P(mask, 1, op, insn); insn.Op1.reg = T0;  insn.Op2.reg = T2;  break;
    case 0x18: insn_1_P(mask, 1, op, insn); insn.Op1.reg = AR0; insn.Op2.reg = AR2; break;
    case 0x1C: insn_1_P(mask, 1, op, insn); insn.Op1.reg = AR4; insn.Op2.reg = T0;  break;
    case 0x1E: insn_1_P(mask, 1, op, insn); insn.Op1.reg = AR6; insn.Op2.reg = T2;  break;
    case 0x2C: insn_1_4(mask, 1, op, insn); insn.Op1.reg = AR4; insn.Op2.reg = T0;  break;
...
static void op_l4(const mask_t *, int64 offset, op_t *op, insn_t &insn) // llll
{
  op->type  = o_near;
...
static void op_L7(const mask_t *, int64 offset, op_t *op, insn_t &insn) // LLLLLLL
{
  op->type  = o_near;
...
static void op_L8(const mask_t *, int64 offset, op_t *op, insn_t &insn) // LLLLLLLL
{
  op->type  = o_near;
...
static void op_L16(const mask_t *, int64 offset, op_t *op, insn_t &insn) // LLLLLLLL LLLLLLLL
{
  op->type  = o_near;
...
static void op_pmad(const mask_t *, int64 offset, op_t *op, insn_t &insn)
{
  op->type  = o_near;
  op->addr  = (get_next_ip(insn) + offset) & 0xFFFFFF;
  op->dtype = dt_code;
...
static void op_P24(const mask_t *, int64, op_t *op, insn_t &insn) // get_byte included
{
  op->type  = o_near;
...
static void op_D16(const mask_t *, int64 offset, op_t *op, insn_t &)
{
  op->type  = o_mem;
  op->addr  = ea_t(offset);
  op->dtype = dt_word;
...
static void op_SHIFTW(const mask_t *, int64 offset, op_t *op, insn_t &)
{
  op->type  = o_shift;
...
//--------------------------------------------------------------------------
// shift left
static void shl_SHIFTW(const mask_t *, int64 offset, op_t *op, insn_t &insn)
{
  get_last_op(op, insn);
...
static void slo_SHIFTW(const mask_t *mask, int64 offset, op_t *op, insn_t &insn)
{
  shl_SHIFTW(mask, offset, op, insn);
  op->tms_shift |= TMS_OP_SHIFT_OUT;
}

//--------------------------------------------------------------------------
// shift right
static void shl_SHFT(const mask_t *, int64 offset, op_t *op, insn_t &insn)
{
  get_last_op(op, insn);
  op->tms_shift       = TMS_OP_SHIFTL_IMM;
...
//--------------------------------------------------------------------------
static void shl_Tx(const mask_t *, int64 offset, op_t *op, insn_t &insn)
{
  get_last_op(op, insn);
...
//--------------------------------------------------------------------------
static void shl_T2(const mask_t *, int64, op_t *op, insn_t &insn)
{
  get_last_op(op, insn);
  op->tms_shift       = TMS_OP_SHIFTL_REG;
...
//--------------------------------------------------------------------------
static void shl_16(const mask_t *, int64, op_t *op, insn_t &insn)
{
  get_last_op(op, insn);
  op->tms_shift       = TMS_OP_SHIFTL_IMM;
...
//--------------------------------------------------------------------------
static void shr(const mask_t *, int64 imm, op_t *op, insn_t &insn)
{
  get_last_op(op, insn);
  op->tms_shift       = TMS_OP_SHIFTR_IMM;
...
//--------------------------------------------------------------------------
static void eq_K16(const mask_t *, int64 offset, op_t *op, insn_t &insn)
{
  get_last_op(op, insn);
...
//--------------------------------------------------------------------------
static void neq_0(const mask_t *, int64, op_t *op, insn_t &insn)
{
  get_last_op(op, insn);
...
//--------------------------------------------------------------------------
static void RELOP(const mask_t *, int64 offset, op_t *op, insn_t &insn) // cc
{
  get_last_op(op, insn);
  op->type      = o_relop;
...
//--------------------------------------------------------------------------
static void RELOP_dst(const mask_t *mask, int64 offset, op_t *op, insn_t &insn)
{
  get_last_op(op, insn);
...
//--------------------------------------------------------------------------
static void RELOP_K8(const mask_t *, int64 offset, op_t *op, insn_t &insn)
{
  get_last_op(op, insn);
...
//--------------------------------------------------------------------------
static void op_cond(const mask_t *mask, int64 offset, op_t *op, insn_t &insn) // CCC CCCC
{
  op->reg = 0;
  if ( (offset>>4) <= 0x5 ) // <= 101 xxxx
  { // src <comparison operator> 0
    op_src(mask, offset & 0xF, op, insn);
    op->type  = o_cond;
    op->value = offset & 0x70;
  }
  else if ( (offset>>2) == 0x18 ) // == 110 00xx
  { // overflow(ACx)
    op_src(mask, offset & 0x3, op, insn);
    op->type  = o_cond;
