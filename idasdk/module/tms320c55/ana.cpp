...
{
public:
  bytes_c() : size(0), i(0) { memset(bytes, 0, sizeof(bytes)); reload(); }
private:
...
static bool byte_match_code_mask(const mask_t *mask, int64 byte, char n, char lbytesize = 8)
{
  ...
  int64 nmask  = 0;
  for (int i = 0; i < lbytesize; i++) nmask = (nmask << 1) | 1;
...
static void process_masks(const mask_t *masks, ushort itype_null, bytes_c &bytes, char lbytesize = 8)
{
  ...
  while ( p->mask != 0 )
  {
    bytes.reset();
    ushort nbytes = 0;
    while ( nbytes < p->size && byte_match_code_mask(p, bytes.get_next(), (uchar)nbytes, lbytesize) )
      nbytes++;
    if ( nbytes == p->size ) // if bytes matches all bytes from code and mask
    {
      ...
      // compute full code
      int64 code = 0;
      for (i = 0; i < nbytes; i++) code = (code << lbytesize) | bytes.get(i);
      // process operands
      int64 op_n = 0;
...
static void get_name_chars(char *buf, size_t bufsize, bool incl_chars, const char *name, const char *chars)
{
...
static bool find_cmd_suffix(const char *chars, ushort itype_last, bool parallel, bool name1_chars, bool name2_chars)
{
  ...
  get_name_chars(cmd_name1_chars, sizeof(cmd_name1_chars), name1_chars, cmd_name1, chars);
  ...
  if ( parallel )
  {
    ...
    get_name_chars(cmd_name2_chars, sizeof(cmd_name2_chars), name2_chars, cmd_name2, chars);
  }
...
static char optional_op; // indice of an optional operand
...
{
  if (optional_op != -1 && cmd.Operands[uchar(optional_op)].type == o_reg
    && cmd.Operands[uchar(optional_op)].reg == AC0 + offset)
...
static void op_mem(const mask_t *mask, int64 offset, op_t &op, char dtyp, bool ARn_mod = false) // AAAAAAAI
{
  ...
  { // direct memory address
    sel_t cpl = get_segreg(cmd.ea, CPL);
    if ( cpl == BADSEL ) cpl = 0;
    if ( !cpl )
    { // use DP
      op.type         = o_mem;
  ...
  { // indirect memory access
    ...
    else
    {
      ...
      else // xxx1 xxx1
      {
        ...
        sel_t arms = get_segreg(cmd.ea, ARMS);
...
{
  op.type = o_near;
  op.addr = (get_next_ip() + offset) & 0xFFFFFF;
  op.dtyp = dt_code;
}
...
{
  op.type = o_near;
  op.addr = (get_next_ip() + get_signed64(offset, 0x7F)) & 0xFFFFFF;
  op.dtyp = dt_code;
}
...
{
  op.type = o_near;
  op.addr = (get_next_ip() + get_signed64(offset, 0xFF)) & 0xFFFFFF;
  op.dtyp = dt_code;
}
...
{
  op.type = o_near;
  op.addr = (get_next_ip() + get_signed64(offset, 0xFFFF)) & 0xFFFFFF;
  op.dtyp = dt_code;
}
          ...
          case BYTE_MMAP: // mmap()
            if ( cmd.OpMem != 0 )
            {
              ...
              if ( cmd.Operands[n].type == o_mem
                && cmd.Operands[n].tms_regH == DPH
                && cmd.Operands[n].tms_regP == DP
                && cmd.Operands[n].tms_modifier == TMS_MODIFIER_DMA )
              { // @dma using DP
              ...
              else if ( cmd.Operands[n].type == o_reg
                     && cmd.Operands[n].reg == SP
