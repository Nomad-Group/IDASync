  ...
  switch ( x.type )
  {
    ...
    case o_displ:
      ...
      if ( may_create_stkvars()
        && !isDefArg(uFlag,x.n)
        && x.indreg == rR7 &&
        !(x.n == 1 && check_insn(0, XA_lea, o_reg, rR7, o_void, BADADDR)))
      {
        ...
        if ( pfn != NULL )
        {
          ...
          if ( decode_insn(cmd.ea+cmd.size) )
          {
            if ( fake.dtyp == dt_word )
            {
              ...
              if (saved.itype == XA_mov && cmd.itype == XA_mov && n == 1 &&   //-V501 identical sub-expressions
                  cmd.Op2.type == o_reg &&
    ...
    case o_phrase:
      if ( x.indreg != rR7 && (x.phrase == fRi || x.phrase == fRip) ) // catch ES:offset references
      {
        ...
        else if ( check_insn(0, XA_setb, o_bit, 0x218+reg, o_void, BADADDR) )
        {
          ...
          { // MOV.B ES/CS,R1L
            ...
            ea_t v = EXTRAMBASE;
            int ok = 0;
            if ( check_insn(1, XA_jb, o_bit, BADADDR, o_void, BADADDR ) &&
          ...
          check_insn(0, XA_mov, o_mem, ES, o_reg, BADADDR) && cmd.Op2.dtyp == dt_byte)
        {
          ...
          if ( check_insn(1, XA_jb, o_bit, BADADDR, o_void, BADADDR ) &&
            ...
            check_insn(1, XA_setb, o_bit, 0x218+reg, o_void, BADADDR))
          {
            int prev = 0;
            if ( check_insn(1, XA_add, o_reg, reg+rR0, o_void, BADADDR) )
              prev = 1;
...
// mov ES,#xx
      if ( check_insn(0, XA_mov, o_mem, ES, o_void, BADADDR) ) // MOV ES,xx
      {
        insn_t saved = cmd;
        ea_t v = 0;
   ...
   if ( decode_insn(prologue) )
   {
     ...
     do
     {
         ...
         cmd.Op2.indreg == rR7)
       {
         ...
         if ( offset >= 0 )
         {
           warning("%a: positive offset %a", cmd.ea, uval_t(offset));
           offset -= 0x10000L;
         }
         frsize -= offset;
         more = true;
      ...
      if ( check_insn(1, XA_add, o_reg, jumpreg, o_imm, BADADDR) )
      {
        ...
        if ( check_insn(1, XA_asl, o_reg, jumpreg, o_imm, 1) )
        {
          if ( check_insn(1, XA_mov, o_reg, rR0H + 2*(jumpreg-rR0), o_imm, 0) )
          {
            ...
            prev = 0;
            if ( check_insn(1, XA_nop, o_void, BADADDR, o_void, BADADDR) )
              prev = 1;
            if ( check_insn(prev, XA_bg, o_void, BADADDR, o_void, BADADDR)
              || (check_insn(0, XA_jmp, o_void, BADADDR, o_void, BADADDR)
               && check_insn(1, XA_bl, o_void, BADADDR, o_void, BADADDR)) )
            {
              if ( check_insn(1, XA_cmp, o_reg, datareg, o_imm, BADADDR) )
              {
                si->ncases = ushort(cmd.Op2.value+1);
                if ( check_insn(1, XA_bcs, o_void, BADADDR, o_void, BADADDR) )
                {
          ...
          {
            prev = 0;
            if ( check_insn(0, XA_nop, o_void, BADADDR, o_void, BADADDR) )
              prev = 1;
            if ( check_insn(prev, XA_bg, o_void, BADADDR, o_void, BADADDR)
              || (check_insn(0, XA_jmp, o_void, BADADDR, o_void, BADADDR)
               && check_insn(1, XA_bl, o_void, BADADDR, o_void, BADADDR)) )
...
void idaapi xa_stkvar_def(char *buf, size_t bufsize, const member_t *mptr, sval_t v)
{
  ...
  qsnprintf(buf, bufsize,
            ...
            inf.indent-1, name.c_str(), sign, vstr);
}
