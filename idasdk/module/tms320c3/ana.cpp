...
//----------------------------------------------------------------------
static bool D_regs(ctx_t &ctx, int value)   // »нтерпретаци€ номеров регистров
{
  if ( (value & 0x1f) > 0x1b )
    return false;
...
//----------------------------------------------------------------------
static bool D_R(ctx_t &ctx, int value)      // »нтерпретаци€ номеров регистров Rx
{
...
//----------------------------------------------------------------------
static bool D_R2(ctx_t &ctx, int value)     // »нтерпретаци€ номеров регистров Rx
{
  return D_regs(ctx, r2 + (value & 0x07));
}

//----------------------------------------------------------------------
static bool D_ar(ctx_t &ctx, int value)     // »нтерпретаци€ номеров регистров arx
{
  return D_regs(ctx, ar0 + (value & 0x07));
}

//----------------------------------------------------------------------
static bool S_ar(ctx_t &ctx, int value)     // »нтерпретаци€ номеров регистров arx
{
  if ( D_ar(ctx, value) )
  {
    ctx.op++;
...
//----------------------------------------------------------------------
static bool D_indir(ctx_t &ctx, int value)  //  освеннна€ адресаци€
{
  ctx.op->type   = o_phrase;
  ctx.op->dtype  = dt_byte;
  ctx.op->phrase = ( value & 0xf800 ) >> 11;                // тип косвеннна€ адресации
  ctx.op->phtype = ar0 + ((value >> 8) & 0x07);             // номер регистра
  ctx.op->addr = value & 0xff;                                              // смещение (если есть)
  return true;
}
...
//----------------------------------------------------------------------
static bool D_indir3(ctx_t &ctx, int value) //  освеннна€ адресаци€ дл€ 3х ќперандных и дл€ паралл команд
{
  ctx.op->type   = o_phrase;
  ctx.op->dtype  = dt_byte;
  ctx.op->phrase = ( value & 0xf8 ) >> 3;   // тип косвеннна€ адресации
  ctx.op->phtype = ar0 + (value & 0x07);    // номер регистра
  ctx.op->addr   = 1;                       // смещение (если есть)
  return true;
...
//----------------------------------------------------------------------
static bool SSD_src3(ctx_t &ctx, int value) // –азбор дл€ 3х-операндных команд
{
...
//----------------------------------------------------------------------
static bool SD_src3_2op(ctx_t &ctx, int value)      // –азбор дл€ 3х-операндных команд, из которых исп. только 2
{
  switch ( (value >> 16) & 0x60 )
  {
    case 0x00:
...
//----------------------------------------------------------------------
static bool DBranch(ctx_t &ctx, int /*value*/)      // ”становить признак отложенного перехода
{
  ctx.insn.auxpref |= DBrFlag;         //delayed branch
  return true;
}

//----------------------------------------------------------------------
static bool ImmFlt(ctx_t &ctx, int /*value*/)       // ”становить признак float константы
{
  ctx.insn.auxpref |= ImmFltFlag;              //delayed branch
  return true;
}

//----------------------------------------------------------------------
static bool cond(ctx_t &ctx, int value)     // »нтерпретаци€ кодов условий
{
...
//----------------------------------------------------------------------
static bool SD_adres(ctx_t &ctx, int value) // √лавна€ адресаци€
{
  switch ( (value >> 16) & 0x60 )
  {
    case 0x00:
...
//----------------------------------------------------------------------
static bool SD_adresRev(ctx_t &ctx, int value)      // √лавна€ адресаци€ с обратным пор€дком операндов
{
  switch ( (value >> 16) & 0x60 )
  {
    case 0x00:
...
//----------------------------------------------------------------------
static bool D_adres_1Op(ctx_t &ctx, int value) // √лавна€ адресаци€ с использованием только одного операнда
{
  switch ( (value >> 16) & 0x60 )
  {
    case 0x00:
...
//--------------------------------------------------------------------------
static int run_functions(ctx_t &ctx, int value, int entry, int start, int end)
{
...
//--------------------------------------------------------------------------
static int run_functions2(ctx_t &ctx, int value, int entry, int start, int end)
{
  const opcode_t &opcode = table[entry];
  for ( int j = start; j <= end; j++ )
  {
...
int idaapi ana(insn_t *insn)
{
  ...
  insn->itype = idx >= qnumber(table) ? TMS320C3X_null : table[idx].itype;
  ...
  if ( insn->itype != TMS320C3X_null )
  {
    ...
    return insn->size;        // длина всегда 1
  }
  else
  {
    return 0;
