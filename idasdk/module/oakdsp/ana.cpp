...
//----------------------------------------------------------------------
static void gen_masks(opcode_t *tbl, int qty)
{
  for ( int i = 0; i < qty; i++ )
  {
    int len = strlen(tbl[i].recog);
    for ( int b = 0; b < len; b++ )
    {
      tbl[i].value <<= 1;
      tbl[i].mask <<= 1;

      if ( tbl[i].recog[b] == '1' || tbl[i].recog[b] == '0' )
        tbl[i].mask++;
    ...
    {
      if ( tbl[i].funcs[j].func )
      {
        for ( int b = 0; b < 32; b++ )
        {
          if ( tbl[i].funcs[j].mask & (1 << b) )
            break;
          else
            tbl[i].funcs[j].shift++;
        }
