...
const char *idaapi set_idp_options(const char *keyword,int value_type,const void *value)
{
  ...
  macro11.XlatAsciiOutput = trans ? trans_dec_pc1 : NULL;
  ash.XlatAsciiOutput = macro11.XlatAsciiOutput;
...
{
  if ( base == ml.ovrtbl_base && value >= ml.ovrcallbeg && value <= ml.ovrcallend )
  {
