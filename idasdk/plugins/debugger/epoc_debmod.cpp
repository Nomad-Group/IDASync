...
//--------------------------------------------------------------------------
bool metrotrk_t::handle_notification(uchar seq, void *_ud) // plugin version
{
  epoc_debmod_t &dm = *(epoc_debmod_t *)_ud;
  int i = 0;
  bool suspend = true;
  debug_event_t ev;
...
int idaapi epoc_debmod_t::dbg_attach_process(pid_t _pid, int /*event_id*/, int /*flags*/)
{
  ...
  trk.get_thread_list(_pid, &threads);
  if ( threads.empty() )
  {
    trk.disconnect();
...
//--------------------------------------------------------------------------
int idaapi epoc_debmod_t::dbg_set_resume_mode(thid_t tid, resume_mode_t resmod)
{
