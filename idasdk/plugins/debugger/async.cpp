...
ssize_t async_stream_t::qrecv(void *buf, size_t n)
{
  ...
  if ( read_data == NULL )
    nomem("async::qrecv, read_size=%x", uint32(read_size));
...
#else
  // lint is right to complain, we have a resource leak here. however,
  // since releasing it hangs, we will have to live with the leak.
  //lint -e{1740} pointer member 'async_stream_t::s' not directly freed or zeroed by destructor
#endif
}
...
static DWORD copy_debugger_server(const char *local, const wchar_t *remote)
{
  ...
  bytevec_t buf;
  buf.resize(size);
  eread(fp, buf.begin(), size);
  qfclose(fp);

  DWORD copied;
  if ( !pCeWriteFile(h, buf.begin(), size, &copied, NULL) || size != copied )
  {
    code = get_errcode();
    pCeCloseHandle(h);
    return code;
...
#define WDLLNAME L"wince_remote_arm.dll"
  ...
  if ( !getsysfile(local_dll, sizeof(local_dll), DLLNAME, "dbgsrv") )
    local_dll[0] = '\0';
