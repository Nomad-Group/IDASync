...
typedef BOOL  WINAPI VirtualProtectEx_t(HANDLE hProcess, LPVOID lpAddress, DWORD dwSize, DWORD flNewProtect, PDWORD lpflOldProtect);
...
typedef DWORD WINAPI CeGetModuleInfo_t(HANDLE hProcess, LPVOID lpBaseAddr, DWORD infoType, LPVOID pBuf, DWORD cbBufSize);
typedef BOOL  WINAPI SetKMode_t(BOOL fMode);
typedef DWORD SetProcPermissions_t(DWORD newperms);

static VirtualQueryEx_t       *_VirtualQueryEx = NULL;
static VirtualProtectEx_t     *_VirtualProtectEx = NULL;
static GetModuleInformation_t *_GetModuleInformation = NULL;
static CeGetModuleInfo_t      *_CeGetModuleInfo = NULL;
static SetKMode_t             *_SetKMode = NULL;
static SetProcPermissions_t   *_SetProcPermissions = NULL;

//--------------------------------------------------------------------------
static void init_cefuncs(void)
{
  HMODULE kern_handle = GetModuleHandle(TEXT(KERNEL_LIB_NAME));
  *(FARPROC*)&_VirtualQueryEx       = GetProcAddress(kern_handle, TEXT("VirtualQueryEx"));
  *(FARPROC*)&_VirtualProtectEx     = GetProcAddress(kern_handle, TEXT("VirtualProtectEx"));
  *(FARPROC*)&_GetModuleInformation = GetProcAddress(kern_handle, TEXT("GetModuleInformation"));
  *(FARPROC*)&_CeGetModuleInfo      = GetProcAddress(kern_handle, TEXT("CeGetModuleInfo"));
  *(FARPROC*)&_SetKMode             = GetProcAddress(kern_handle, TEXT("SetKMode"));
  *(FARPROC*)&_SetProcPermissions   = GetProcAddress(kern_handle, TEXT("SetProcPermissions"));
}
...
//--------------------------------------------------------------------------
static int get_module_by_base(debmod_t * /*sess*/, MODULEENTRY32 *me32, void *ud)
{
  modbase_to_entry_t *mbh = (modbase_to_entry_t*)ud;
  if ( (ea_t)(me32->modBaseAddr) == mbh->modbase )
  {
    mbh->me = *me32;
    return 1;
  }
    ...
    )
{
  ...
  if ( _VirtualQueryEx )
    return _VirtualQueryEx(hProcess, lpAddress, lpBuffer, dwLength);
