...
public:
  ...
  rangeset_t(void) : cache(NULL) {}                                                          ///< Constructor
  rangeset_t(const range_t &range): cache(NULL) { if ( !range.empty() ) bag.push_back(range); }  ///< Constructor - Initialize set with 'range'
  rangeset_t(const rangeset_t &ivs) : bag(ivs.bag), cache(NULL) {}                            ///< Constructor - Initialize set with 'ivs'
  rangeset_t &operator=(const rangeset_t &ivs) { bag = ivs.bag; cache = NULL; return *this; }
  void swap(rangeset_t &r) { bag.swap(r.bag); std::swap(cache, r.cache); }                                              ///< Set this = 'r' and 'r' = this. See qvector::swap()
  ...
  /// \param range  address range to add. can not be empty
  ...
  /// \param range  address range to subtract. can not be empty.
